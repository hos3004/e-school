...
final class CompleteProfileController extends Controller
{
    ...
    public function store(CompleteProfileRequest $request, AccountProfile $profiles, AuditRecorder $audit, GeographyQueries $geo): RedirectResponse
    {
        ...
        DB::transaction(function () use ($actor, $profiles, $data, $audit, $geo): void {
            ...
            $before = ['account' => $user->only(['name', 'email', 'phone', 'phone_country', 'timezone', 'profile_completed_at']), 'student' => $student?->only(['country_id', 'region_id', 'region_name', 'city', 'date_of_birth', 'gender']), 'teacher' => $teacher?->only(['country_id', 'region_id', 'region_name', 'city', 'date_of_birth', 'gender', 'phone'])];
