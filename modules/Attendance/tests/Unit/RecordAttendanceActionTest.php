...
it('records attendance with the derived status and publishes the event', function () {
...
it('rejects recording attendance twice for the same participant', function () {
...
it('rejects a non-positive session duration', function () {
    app(RecordAttendanceAction::class)->execute(
        sessionParticipantId: (string) str()->ulid(),
        attendedMinutes: 30,
        sessionMinutes: 0,
...
it('rejects an empty participant identifier', function () {
