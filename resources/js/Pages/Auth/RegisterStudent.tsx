...
export default function RegisterStudent({
  ...
  const questionField = (question: EvalQuestion) => {
        ...
        >
          ...
          <div className="space-y-2">
            {(question.options ?? []).map((option) => (
              <label
                key={option}
                className="flex min-h-11 items-center gap-3 rounded-[var(--radius-md)] border border-[var(--line)] px-3 text-sm text-[var(--ink)] hover:bg-[var(--surface-subtle)] focus-within:border-[var(--brand)] focus-within:ring-2 focus-within:ring-[var(--focus-ring)]"
              >
                <input
                  type={question.type}
    ...
    return (
      <label
        key={question.id}
        className="block text-sm font-semibold text-[var(--ink)]"
        htmlFor={`evaluation-${question.id}`}
      >
        {legend}
  ...
  const content = (
    <>
      ...
      <div className={consoleRegistration ? "pr-heading" : "text-center"}>
        {consoleRegistration && (
          <span className="pr-eyebrow">{t("public_registration.eyebrow")}</span>
        )}
        <h1 className="text-2xl font-semibold leading-tight tracking-[-0.02em] text-[var(--ink)] [text-wrap:balance]">
      ...
      {form.hasErrors && (
        <div
          ...
          role="alert"
          tabIndex={-1}
          className="mt-5 rounded-[var(--radius-md)] border border-[color:var(--danger)]/30 bg-[var(--danger-soft)] p-3 text-sm font-medium text-[var(--danger)]"
        >
      ...
      <form
        className={consoleRegistration ? "pr-form" : "mt-7 space-y-6"}
        onSubmit={submit}
      >
        <fieldset
          ...
          className={
            consoleRegistration
              ? "pr-section pr-personal"
              : "space-y-4 rounded-[var(--radius-lg)] border border-[var(--line)] bg-[var(--surface-subtle)] p-5"
          }
        >
          <legend className="px-2 text-sm font-semibold text-[var(--ink)]">
            {consoleRegistration && <span className="pr-step">01</span>}
          ...
          >
            ...
            <select
              ...
              onChange={(event) =>
                form.setData("region_id", event.target.value)
              }
            >
              <option value="">{t("auth.register.choose")}</option>
              {(regions[form.data.country_id] ?? []).map((region) => (
        ...
        <fieldset
          ...
          className={
            consoleRegistration
              ? "pr-section pr-contact"
              : "space-y-4 rounded-[var(--radius-lg)] border border-[var(--line)] bg-[var(--surface-subtle)] p-5"
          }
        >
          <legend className="px-2 text-sm font-semibold text-[var(--ink)]">
            {consoleRegistration && <span className="pr-step">02</span>}
            {t("auth.register.contact_information")}
          </legend>
          <p className="text-xs text-[var(--ink-muted)]">
            {t("auth.register.contact_help")}
          </p>
        ...
        {questions.length > 0 && (
          <fieldset
            id="registration-evaluation"
            className={
              consoleRegistration
                ? "pr-section pr-evaluation"
                : "space-y-5 rounded-[var(--radius-lg)] border border-[var(--line)] bg-[var(--surface-subtle)] p-5"
            }
          >
            <legend className="px-2 text-sm font-semibold text-[var(--ink)]">
              {consoleRegistration && <span className="pr-step">03</span>}
        ...
        <label
          className={
            consoleRegistration
              ? "pr-notes"
              : "block text-sm font-semibold text-[var(--ink)]"
          }
          htmlFor="notes"
        >
        ...
        <div className={consoleRegistration ? "pr-submit-row" : undefined}>
          {consoleRegistration && <p>{t("public_registration.review_hint")}</p>}
          <Button
            disabled={form.processing}
            fullWidth={!consoleRegistration}
            type="submit"
          >
            {form.processing
  ...
  if (!consoleRegistration) return <GuestLayout>{content}</GuestLayout>;
  return (
      ...
      sidebar={
        <>
          <section className="pr-summary">
            ...
            <h2>{t("public_registration.summary")}</h2>
            <p>{registrationForm.title}</p>
            {registrationForm.course || registrationForm.program ? (
              <dl>
                {registrationForm.course && (
                  <div>
                    <dt>{t("public_registration.course")}</dt>
                    <dd>{registrationForm.course}</dd>
                  </div>
                )}
                {registrationForm.program && (
                  <div>
                    <dt>{t("public_registration.program")}</dt>
                    <dd>{registrationForm.program}</dd>
                  </div>
                )}
              </dl>
            ) : (
              <dl>
                <div>
                  <dt>{t("public_registration.general")}</dt>
                  <dd>{t("public_registration.general_hint")}</dd>
                </div>
              </dl>
            )}
          </section>
          <section className="pr-help-card">
            <h2>{t("public_registration.complete")}</h2>
            <ul className="pr-progress-list">
              <li>
              ...
              {questions.length > 0 && (
                <li>
                  <CheckCircle2 size={17} />
                  <a href="#registration-evaluation">
                    {t("auth.register.evaluation_questions")}
                  </a>
                </li>
              )}
          ...
          <p className="pr-summary-note">
            <ShieldCheck size={18} />
            <span>{t("public_registration.private_hint")}</span>
          </p>
        </>
      }
    >
