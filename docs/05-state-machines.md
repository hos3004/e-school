...
| السبب | ظرف شخصي | ثلاث مخالفات في الشهر |
...
stateDiagram-v2
    ...
    RuleEvaluation --> CountWithinWindow : كم مخالفة هذا الشهر؟
...
```
العدّاد = عدد مخالفات هذا القيد داخل الشهر الميلادي الحالي
```

يُصفَّر أول كل شهر (`discipline.counter_reset_day = 1`).
**التصفير لا يحذف السجلات** — يغيّر نطاق الاحتساب فقط. سجل المخالفات
التاريخي كامل ودائم ويظهر في ملف الطالب.
