...
## عزل البيئة — شرط قبل أي Suite

- لا يُشغّل اختبار على قاعدة development أو production، ولا على اسم قاعدة مشترك بين وكيلين.
- لكل run/agent PostgreSQL database مستقل، وRedis prefix/database وqueues وstorage وmail مستقلة.
- Guard يفشل سريعًا إذا لم تكن `APP_ENV=testing` أو طابق اسم القاعدة قاعدة التشغيل.
- ملفات إعداد محلية مستثناة من Git ليست بديلًا عن إعداد موثق يعمل في CI.
- لا Gates أو permissions مصطنعة داخل الاختبار لتجاوز إعداد الإنتاج.
- سيناريو مرحلة أولى إلزامي لا يقبل `todo()` أو `skip`; الاعتماد الخارجي غير المتوفر
  يسجل في مجموعة external كـBlocked، ولا يحول بقية الرحلة إلى نجاح وهمي.
- Playwright الليلي يُشغّل أيضًا يدويًا كجزء من بوابة تسليم المرحلة الأولى.
...
```
        ▲   Playwright  ~14 رحلة       بطيئة · غالية · لا غنى عنها
       ╱ ╲
...
### الحسابات المالية — suite مستقبلية خارج قبول Phase 1

تبقى اختبارات سلامة كود المال الموجود ولا تُعطّل أو تُخفّف، لكنها ليست رحلة تسليم
المرحلة الأولى ولا تبرر إظهار Payroll. عند بدء مرحلة المال تُنفّذ أمثلة
[`14-payroll-rules.md`](14-payroll-rules.md) حرفيًا:
...
it('مستمع الإشعار idempotent — استدعاؤه مرتين لا يضاعف الصندوق', function () {
    $listener->handle($event);
    $listener->handle($event);

    expect(DB::table('notification_outbox')->where('idempotency_key', $key)->count())
        ->toBe(1);
});
```
...
رحلات Phase 1 الأغلى والأهم:

1. الحسابات الأربعة تدخل واجهاتها، ومنح/سحب الصلاحية يسري في الطلب التالي.
2. Admin ينشئ Program وتصنيفًا ودورة ومجموعة بعلاقاتها.
3. التسجيل العام → مراجعة → قبول → ملف طالب → قيد → تسكين.
4. استيراد Excel للبيانات الحالية يمر بالقواعد نفسها ولا يضاعف الصفوف.
5. الإتاحة والجدولة تنشئان حصة وتمنعان تعارض الطالب والمعلم والمجموعة.
6. Teacher/Student/Supervisor يدخلون BBB بالأدوار الصحيحة وتُرفض المؤسسة الأخرى.
7. التسجيل وأحداث join/leave تعود، ويُعتمد الحضور ويكتب المعلم التقرير الفوري.
8. الإلغاء والتأجيل والاعتذار والبديل يحفظون actor والسبب والتاريخ الصحيح.
9. الغياب المتكرر يرسل التنبيهات ويجمّد الطالب، ثم تقييم إعادة التفعيل.
10. التذكير يصل In-App وEmail وWhatsApp مرة واحدة، واستعادة phone-only تعمل.
11. المراسلة الخاصة تعمل وتمنع Guardian/غير المصرّح من URL المباشر.
12. شات المجموعة يمنع عضو مجموعة أو مؤسسة أخرى والطالب المجمّد وفق السياسة.
13. تكليف يُنشر ويُسلّم ويُراجع مع عزل submission والمرفقات.
14. التقارير والتصدير تعكس المؤسسة الصحيحة، مع العربية RTL وعرض هاتف/سطح مكتب.
